...
def complex_solve(num_1, num_2, oper = '+'): # нахождение результата выражения из двух комплексных чисел
    ...
    if oper == '/':
        if num_1[1] < 0:
            try: result[0] = (num_1[0] * num_2[0] + num_1[1] * num_2[1]) / (num_2[0] ** 2 + num_2[1] ** 2)
            except ZeroDivisionError: return None

            try: result[1] = (num_1[0] * -num_2[1] + num_1[1] * num_2[0]) / (num_2[0] ** 2 + num_2[1] ** 2)
            except ZeroDivisionError: return None
        else:
            try: result[0] = (num_1[0] * num_2[0] + num_1[1] * num_2[1]) / (num_2[0] ** 2 + num_2[1] ** 2)
            except ZeroDivisionError: return None
